//! Integration tests for petgraph-cypher.

use petgraph_cypher::{
    ast::Clause, build_graph_from_cypher, parse_cypher, CypherError, CypherValue, Expression,
    NodeData, RelDirection,
};
...
fn parse_match_with_where() {
    ...
    let petgraph_cypher::WhereExpr::Eq(expr, val) = where_clause.as_ref().unwrap() else {
